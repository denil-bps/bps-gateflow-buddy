...
function PublicDisplay() {
  ...
  return <main className={`min-h-screen ${tone} flex items-center justify-center overflow-hidden p-8 text-primary-foreground transition-colors duration-700`}>
    <div className="absolute inset-0 opacity-30" style={{ backgroundImage: "radial-gradient(circle at 15% 20%, var(--color-brand-sky), transparent 28%), radial-gradient(circle at 85% 80%, var(--color-brand-gold), transparent 24%)" }} />
    <div className="relative mx-auto flex w-full max-w-6xl flex-col items-center text-center">
      ...
      {state.safe_name && <p className="mt-7 text-3xl font-semibold sm:text-4xl">{state.safe_name}</p>}
